...
exports.analyzeMultipleImages = exports.analyzePlantDisease = void 0;
...
function generateCacheKey(base64Images, options) {
...
function normalizeSingleResult(raw, index) {
    const parsed = (raw ?? {});
    const causes = Array.isArray(parsed.causes)
        ? parsed.causes.filter((cause) => typeof cause === 'string' && cause.trim().length > 0).slice(0, 5)
        : [];
    const treatmentSteps = Array.isArray(parsed.treatment_steps)
        ? parsed.treatment_steps.filter((step) => typeof step === 'string' && step.trim().length > 0).slice(0, 6)
        : [];
    return {
        plant_name: typeof parsed.plant_name === 'string' && parsed.plant_name.trim()
        ...
        treatment: typeof parsed.treatment === 'string' && parsed.treatment.trim()
        ...
        prevention: typeof parsed.prevention === 'string' && parsed.prevention.trim() ? parsed.prevention.trim() : undefined,
        severity: normalizeSeverity(parsed.severity),
        causes: causes.length > 0 ? causes : undefined,
        treatment_steps: treatmentSteps.length > 0 ? treatmentSteps : undefined,
        additional_info: typeof parsed.additional_info === 'string' && parsed.additional_info.trim()
            ? parsed.additional_info.trim()
            : undefined,
    };
}
function normalizeResults(raw, imageCount) {
    ...
    return Array.from({ length: imageCount }, (_, index) => normalizeSingleResult({
...
function buildPrompt(imageCount, options) {
...
async function analyzeWithOpenRouter(base64Images, options = {}) {
...
const analyzePlantDisease = async (imageBuffer, options = {}) => {
...
exports.analyzePlantDisease = analyzePlantDisease;
const analyzeMultipleImages = async (images, options = {}) => {
    if (images.length === 0) {
        return [];
    }
    return (0, exports.analyzePlantDisease)(images, options);
};
