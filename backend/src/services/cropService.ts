...
const fetchCropImage = async (cropName: string): Promise<{ image_url?: string; image_alt?: string }> => {
  ...
  try {
    ...
    const data = await response.json() as {
      photos?: Array<{ src?: { medium?: string; large?: string }; alt?: string }>;
    };
...
const getWikimediaCropImage = (cropName: string): { image_url: string; image_alt: string } => {
  const normalizedName = cropName.toLowerCase().trim();
  
  // Wikimedia Commons images (public domain/CC licensed)
  const wikimediaImages: Record<string, { url: string; alt: string }> = {
    rice: {
...
export const generateCropRecommendations = async (soilData: SoilData): Promise<CropRecommendation[]> => {
...
Format as JSON array of objects with keys: crop_name, reason, planting_month, care_tips. Return ONLY the JSON array, no other text.`;
  ...
  try {
    ...
    const parsed = JSON.parse(aiResponse) as CropRecommendation[];
    const recommendations: CropRecommendation[] = Array.isArray(parsed) ? parsed : [];
...
export const generateLocationCropRecommendations = async (locationData: LocationData): Promise<CropRecommendation[]> => {
...
Format as JSON array of objects with keys: crop_name, reason, planting_month, care_tips. Return ONLY the JSON array, no other text.`;
  ...
  try {
    ...
    const parsed = JSON.parse(extractJsonBlock(aiResponse)) as CropRecommendation[];
    const recommendations: CropRecommendation[] = Array.isArray(parsed) ? parsed : [];
...
function getLocationFallbackCrops(locationData: LocationData): CropRecommendation[] {
...
function normalizeFarmingGuide(raw: unknown, cropName: string): FarmingGuide {
  const parsed = (raw ?? {}) as Partial<FarmingGuide> & {
    steps?: Partial<FarmingGuideStep>[];
    tips?: unknown;
  };
  ...
  const tips = Array.isArray(parsed.tips)
    ? parsed.tips.filter((tip): tip is string => typeof tip === 'string' && tip.trim().length > 0).slice(0, 5)
    : [];
  ...
  return {
    ...
    climate:
      typeof parsed.climate === 'string' && parsed.climate.trim()
        ? parsed.climate.trim()
        : `Match ${cropName} production to the local temperature, rainfall, and sunlight pattern in your area.`,
    soilPreparation:
      typeof parsed.soilPreparation === 'string' && parsed.soilPreparation.trim()
    ...
    harvestTime:
      typeof parsed.harvestTime === 'string' && parsed.harvestTime.trim()
        ? parsed.harvestTime.trim()
        : 'Depends on variety and local conditions',
    expectedYield:
      typeof parsed.expectedYield === 'string' && parsed.expectedYield.trim()
...
export const generateFarmingGuide = async (
  cropName: string,
  context?: { reason?: string; plantingMonth?: string; careTips?: string }
